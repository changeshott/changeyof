...
export default function FeedbackReactionsPage() {
  const [hoveredFeature, setHoveredFeature] = useState<number | null>(null);

  const features = [
    {
      title: "Universal Inbox",
      desc: "Aggregate feedback from email, widgets, and in-app forms into a single view.",
      icon: Inbox
    },
    {
      title: "Emoji Reactions",
      desc: "Let users quickly express their feelings with customizable emoji reactions.",
      icon: Smile
    },
    {
      title: "Sentiment Analysis",
      desc: "Automatically categorize feedback as positive, neutral, or negative using AI.",
      icon: Activity
    },
    {
      title: "Threaded Replies",
      desc: "Engage in direct conversations with users to clarify their feedback.",
      icon: MessageCircle
    },
    {
      title: "Saved Replies",
      desc: "Respond to common feedback faster with team-wide saved templates.",
      icon: Send
    },
    {
      title: "Anomaly Detection",
      desc: "Get alerted immediately if negative sentiment spikes unexpectedly.",
      icon: AlertCircle
    }
  ];

  const steps = [
    { title: "Capture", desc: "Users leave feedback anywhere in your app." },
    { title: "Triage", desc: "Feedback is automatically categorized by sentiment." },
    { title: "Engage", desc: "Reply directly or add reactions to acknowledge." },
    { title: "Resolve", desc: "Turn feedback into actionable product updates." }
  ];
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-[#fafafa] selection:bg-white/20 overflow-hidden flex flex-col">
      ...
      <section className="relative z-10 w-full max-w-7xl mx-auto px-6 pt-40 md:pt-48 pb-24 text-center flex-grow flex flex-col items-center justify-center">
        ...
        >
          Understand the Pulse of Your Users
        </motion.h1>
        ...
        >
          Comments, reactions, and sentiment at a glance. Centralize qualitative feedback and respond faster to what matters.
        </motion.p>
        ...
        >
          <Link href="/signup" className="w-full sm:w-auto group relative px-8 py-3.5 bg-white text-black rounded-full font-medium hover:scale-105 transition-all flex items-center justify-center gap-2 overflow-hidden">
             <span className="relative z-10">Start gathering feedback</span>
          ...
          <div className="bg-[#0a0a0a] rounded-2xl border border-white/5 p-6 min-h-[400px] flex flex-col relative z-10">
            <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/5">
               <div className="flex items-center gap-4">
                 ...
                 <div>
                   <h3 className="font-medium text-white">Feedback Stream</h3>
                   <p className="text-sm text-slate-500">Live Incoming Data</p>
                 </div>
               </div>
            ...
            <div className="space-y-4 flex-grow">
              {[
                { user: "Alex T.", comment: "The new dashboard is incredible! Much faster than before.", sentiment: "Positive", time: "2m ago", icon: Heart },
                { user: "Sarah M.", comment: "I'm having trouble finding the export button. Is it moved?", sentiment: "Neutral", time: "15m ago", icon: Inbox },
                { user: "John D.", comment: "Integration with slack keeps failing for me today.", sentiment: "Negative", time: "1h ago", icon: AlertCircle }
              ].map((item, i) => (
                <div key={i} className="flex gap-4 p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.04] transition-colors">
                  <div className="w-10 h-10 rounded-full bg-white/10 border border-white/10 flex items-center justify-center shrink-0">
                    <span className="text-white/60 text-sm font-medium">{item.user.charAt(0)}</span>
                  </div>
                  <div className="flex-grow">
                    <div className="flex justify-between items-start mb-1">
                      <h4 className="font-medium text-white/90 text-sm">{item.user}</h4>
                      <span className="text-xs text-slate-500">{item.time}</span>
                    </div>
                    <p className="text-sm text-slate-300 mb-3">{item.comment}</p>
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`text-[10px] uppercase tracking-wider font-semibold px-2 py-0.5 rounded-full ${
                        item.sentiment === 'Positive' ? 'bg-white/20 text-white' : 
                        item.sentiment === 'Negative' ? 'bg-white/5 border border-white/20 text-slate-400' : 
      ...
      <section className="relative z-10 w-full max-w-7xl mx-auto px-6 py-24 border-t border-white/5">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-4 text-white">Listen and Respond at Scale</h2>
          <p className="text-slate-400 max-w-2xl mx-auto">Don&apos;t let valuable qualitative feedback get lost in the noise.</p>
        </div>
      ...
      <section className="relative z-10 w-full max-w-7xl mx-auto px-6 py-24 border-t border-white/5">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-4 text-white">Built for High Volume</h2>
          <p className="text-slate-400 max-w-2xl mx-auto">Tools designed to help product teams digest thousands of comments easily.</p>
        </div>
      ...
      <section className="relative z-10 w-full max-w-5xl mx-auto px-6 py-32 text-center">
         <div className="glass p-12 md:p-20 rounded-[3rem] border border-white/10 relative overflow-hidden group">
           ...
           <h2 className="text-4xl md:text-5xl font-bold mb-6 tracking-tight text-white relative z-10">Stop guessing. Start listening.</h2>
           <p className="text-slate-400 mb-10 max-w-xl mx-auto relative z-10 text-lg">Centralize your qualitative feedback and start making product decisions backed by real user sentiment.</p>
