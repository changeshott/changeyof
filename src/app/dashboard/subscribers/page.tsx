...
export default async function SubscribersPage(props: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
    ...
    return (
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        <AnimatedHeader 
          ...
          description="Build your release notes mailing list." 
        />
        <div className="bg-[#111] border border-white/10 rounded-2xl p-10 text-center mt-6">
          <h3 className="text-xl font-semibold text-white mb-2">No Projects Yet</h3>
          <p className="text-slate-400">You need to create a project first before you can manage subscribers.</p>
        </div>
      </main>
    );
  }
    ...
    return (
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        <AnimatedHeader 
          ...
          description="Choose a project to view its subscribers." 
        />
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project) => (
            <Link 
              key={project.id} 
              href={`/dashboard/subscribers?projectId=${project.id}`}
  ...
  if (!selectedProjectId && projects.length === 1) {
    redirect(`/dashboard/subscribers?projectId=${projects[0].id}`);
  }

  const initialSubscribers = await getSubscribers(activeProject.id);
  ...
  return (
    <main className="max-w-6xl mx-auto flex flex-col px-4 sm:px-6 py-6 pb-20 min-h-screen">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <AnimatedHeader 
          title="Subscribers" 
          description={`Manage your mailing list for ${activeProject.name}`} 
        />
        {projects.length > 1 && (
          <Link 
      ...
      <SubscribersClient projectId={activeProject.id} initialSubscribers={initialSubscribers} />
    </main>
  );
}
