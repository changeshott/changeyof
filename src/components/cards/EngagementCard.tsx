...
export function EngagementCard({ onClick }: { onClick?: () => void }) {
  ...
  return (
    <motion.div 
      ...
      className="group relative glass p-5 rounded-2xl w-56 shadow-[0_20px_40px_rgba(0,0,0,0.5)] cursor-none flex flex-col bg-gradient-to-br from-white/[0.05] to-transparent border border-white/10 hover:border-white/20 hover:bg-white/[0.08] transition-all duration-300"
    >
      <div className="absolute -inset-2 bg-gradient-to-r from-white to-teal-500 rounded-[24px] blur-xl opacity-0 group-hover:opacity-20 transition-opacity duration-500 -z-10"></div>

      <div className="flex justify-between items-start mb-4 relative z-10">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-white/10 to-teal-500/20 flex items-center justify-center text-emerald-300 border border-white/30 shadow-[0_0_15px_rgba(16,185,129,0.3)] group-hover:shadow-[0_0_25px_rgba(16,185,129,0.6)] transition-shadow">
          <Activity size={20} className="group-hover:scale-110 transition-transform" />
        </div>
        <div className="flex items-center gap-1 text-white text-xs font-medium bg-white/5 px-2 py-1 rounded-full border border-white/10 group-hover:bg-white/10 transition-colors">
          <TrendingUp size={12} /> +24%
        </div>
      </div>
      <div className="relative z-10">
        <div className="flex items-center gap-2">
           <h3 className="text-3xl font-bold text-white tracking-tighter group-hover:text-emerald-300 transition-colors">98.4<span className="text-white text-xl">%</span></h3>
