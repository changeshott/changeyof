...
export function WhatsNewCard({ onClick }: { onClick?: () => void }) {
  ...
  return (
    <motion.div 
      ...
      className="group relative glass p-3 rounded-2xl w-64 shadow-[0_20px_40px_rgba(0,0,0,0.5)] flex items-center gap-4 cursor-none border border-white/10 bg-gradient-to-br from-slate-400/[0.05] to-transparent hover:border-white/20 hover:bg-white/[0.1] transition-all duration-300"
    >
      <div className="absolute -inset-2 bg-gradient-to-r from-slate-400 to-orange-500 rounded-[24px] blur-xl opacity-0 group-hover:opacity-25 transition-opacity duration-500 -z-10"></div>

      <div className="relative">
        <div className="absolute inset-0 bg-slate-400/30 blur-xl rounded-full opacity-50 group-hover:opacity-100 transition-opacity" />
        <div className="relative w-14 h-14 rounded-2xl bg-[url('https://images.unsplash.com/photo-1550684848-fac1c5b4e853?q=80&w=200&auto=format&fit=crop')] bg-cover bg-center border border-slate-400/30 flex items-center justify-center text-white shadow-lg overflow-hidden group-hover:border-slate-300/50 transition-colors">
          <div className="absolute inset-0 bg-slate-400/20 backdrop-blur-sm mix-blend-overlay group-hover:scale-125 transition-transform duration-1000 ease-out"></div>
      ...
      <div className="flex-1 relative">
        <div className="flex items-center gap-1.5 mb-0.5">
           ...
           <Sparkles size={12} className="text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity" />
        </div>
